    ...
    return (
        <nav className="flex justify-between items-center rounded-md bg-fuchsia-200 pl-4 pr-3 py-2 mx-12 my-4">
            ...
            <div className="flex items-center space-x-4">
                ...
                <button
                    className="flex  items-center gap-1 bg-fuchsia-500 outline-none text-white px-2 py-2 rounded-md cursor-pointer hover:bg-fuchsia-600"
                >
                    {isLoggedIn ? " Logout" : " Join Us"}
                    {isLoggedIn ? <LogOut size={18} /> : <UserPlus size={18} />}
                </button>
            </div>
        </nav>
    )
